...
#### D. 그래프 모델링 기법 비교 (Recommendation)
데이터 성격에 따른 적합한 모델링 기법 비교표입니다.

| 기법 | 적합한 데이터 | 검색 질문 예시 | 특징 |
| :--- | :--- | :--- | :--- |
| **개념 중심 (Concept)** <br> *(Current POC)* | 정책 보고서, 백서, 법령 | "스토킹 범죄의 정의가 뭐야?" | 주어-서술어-목적어 트리플 구조. <br> 정의 및 관계 파악에 유리. |
| **사건 중심 (Event)** | 수사 보고서, 조서, 112 신고 | "김철수가 어제 강남역에서 뭐 했어?" | 사건을 노드로, 5W1H를 속성으로 연결. <br> 복잡한 시공간 정보 표현 가능. |
| **계층 중심 (Taxonomy)** | 조직도, 법률 조항 체계 | "여성청소년계 업무에 속하는 범죄들은?" | 상/하위 포함 관계(IS-A) 중심. <br> 카테고리 추론에 강력함. |
| **문서 구조 (Document)** | 증거 자료, 매뉴얼 | "이 매뉴얼 3장에서 말하는 내용은?" | 문서-챕터-청크-엔티티 연결. <br> 답변의 출처(Provenance) 추적에 최적. |

> **결론**: 향후 실제 수사 지원 시스템 구축 시, **[문서 구조 + 엔티티(사건/개념)]**를 결합한 **하이브리드 기법** 도입을 권장합니다.
